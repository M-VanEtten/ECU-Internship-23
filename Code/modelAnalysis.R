...
createSpeciesDataset <- function() {
  ...
  beringSeaCTD <- read.csv("data/BASIS_CTD_48e9_1d95_4312.csv") %>%
    ...
    summarize(surface_temp_oC = mean(primary_temperature), surface_sal_psu = mean(primary_salinity))
  ...
  speciesData <-  mutate(speciesData, presence = replace_na(presence, 0),
    ...
    subset(sst != 0 & salinity != 0 & ssh_roms != 0) %>% # remove lower outliers
    subset(sst <= 40 & salinity <= 40) %>%  # remove upper outliers
...
dataSablefishSDM <- subset(dataSablefishSDM, latitude > 41)

#create mesh -- use make_mesh()-------------------------------------------------
mesh <- make_mesh(dataSablefishSDM, xy_cols = c("X", "Y"), n_knots = 200)

#fit the model -- use sdmTMB()--------------------------------------------------
fitSablefish <- sdmTMB(formula = logN1 ~ s(sst_scaled) + s(ssh_scaled) + as.factor(gearGeneral),
                       spatial = "on",
